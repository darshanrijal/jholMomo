...
export const settings = async (values: z.infer<typeof settingsSchema>) => {
  ...
  if (values.password && values.newPassword && dbUser.password) {
    ...
    const hashedPassword = await bcrypt.hash(values.newPassword, 10); // Hash the new password
    values.password = hashedPassword;
    values.newPassword = undefined; // Clear newPassword as it's not needed anymore
  }
